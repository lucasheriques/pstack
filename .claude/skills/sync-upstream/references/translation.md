...
- `skills/poteto-mode/scripts/merge-gate/` is port-only. It reuses watch-pr's readiness verdict without editing watch-pr, and adds the policy: allowed repos, human-only paths needing a human approval on the head, and head pinning with `--match-head-commit`. Opening a PR step 4 and babysit merge only through it. The policy lives outside git, so an agent cannot widen it through a PR.
