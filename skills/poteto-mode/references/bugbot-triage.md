...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles comments from review bots such as Copilot, Greptile, CodeRabbit, or a security-review bot. The goal is not to ignore review bots by default. The goal is to stop treating every comment as a required code change.
...
- Confidence: candidate
- Skip when: The bot flags an export, component, helper, or file as unused, and the PR list and diffs, upper-stack diffs, or PR context show it is used by a later PR in the stack.
...
- Confidence: candidate
- Skip when: A security-review bot (or similar) claims a missing authz/validation call, and the current PR tip clearly includes that exact gate (with tests), typically added in a later hardening commit after the review ran.
