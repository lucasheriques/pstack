...
- **Worker / verifier.** A background subagent with `isolation: "worktree"`. Use `isolation: "remote"` only when the unit must outlive this session, and never when it needs this machine: browser or `run`-skill runtime verification, local transcripts under `~/.claude/projects/`, simulators and local IDE state, or auth that exists only here. Remote agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model from its worker.
