...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned and drained only through the Agent tool, and resumed only through `SendMessage`. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (a nested spawn has the full Agent tool schema, including `isolation`). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** A background subagent with `isolation: "remote"`, so it survives a restart of this session, unless the unit needs this machine: browser or `run`-skill runtime verification, local transcripts under `~/.claude/projects/`, simulators and local IDE state, or auth that exists only here. Those run locally with `isolation: "worktree"`. Remote agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model from its worker.
...
FORBIDDEN    no gh stack, no rebase, no force-push, no fixes outside scope, plus unit-specific bans
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the isolation default and the local-only exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from the stack's own tracking (`gh stack view --json` in the stacker's clone) after every merge and stack mutation, because base refs drift mid-restack: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR.
- Exactly one stacker per stack may run `gh stack` mutations (`rebase`, `sync`, `push`, `submit`, `link`), serialized within its stack. Record the holder in the standing orders. Restacks run remote (`isolation: "remote"`). A local restack at this scale takes the laptop down.
- Workers never rebase and never run `gh stack`. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation. They report conflicts to the stacker rather than restacking.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run. A worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one worktree or remote session when it dies was never done.

#### Liveness and failure

- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, a remote agent's session status. Transcript mtime is not liveness.
...
- After a Claude Code restart: local background agents are dead, remote work is not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach remote work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
