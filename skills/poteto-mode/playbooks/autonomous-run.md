...
2. Pick the wake mechanism with the built-in `/loop` skill. An event to watch (CI, a merge, a ref advancing) gets a background watcher (a `run_in_background` command or a background subagent) that wakes you when it returns, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
