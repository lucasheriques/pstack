...
4. Merge, when this PR is the whole task rather than a layer mid-stack or mid-phase. Run the **Babysit** playbook in `drive` mode until the watcher reports `READY`, then `scripts/merge-gate/merge-gate <number> --merge`. Never run `gh pr merge` yourself. A final refusal is never overridden that way: the user merges it.
   ...
   - After a merge, read the merge commit from the verdict, or from `gh pr view` when the verdict's is null. Wait for trunk's checks and every deploy workflow the merge commit triggers. A run that a later trunk merge cancelled or superseded is never red: it counts as green once the same workflow is green on a later trunk commit that contains the merge commit. If this change turned trunk or a deploy red, tell the user at once. Then open a revert PR through these steps, resolve its bot threads yourself unless one reports a deploy or data-compatibility risk (that one goes to the user), and merge it through the gate as soon as it accepts, without waiting for human reviewers. If the gate refuses the revert for a final reason, the revert is the user's.
