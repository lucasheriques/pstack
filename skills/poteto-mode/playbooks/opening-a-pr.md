...
4. Merge, when this PR is the whole task rather than a layer mid-stack or mid-phase. Run the **Babysit** playbook in `drive` mode until the watcher reports `READY`, then `scripts/merge-gate/merge-gate <number> --merge`, never `gh pr merge` directly.
   - `needs-human-approval`, `repo-not-allowed`, `not-author`, `changes-requested`, `base-not-trunk`, and `policy-invalid` are final: report the reason and any listed files, and leave the merge to the user.
   - For `checks-pending`, `not-mergeable`, `head-moved`, or `github-error`, wait for the watcher and rerun once, then report.
   - For `merge-unverified`, read the PR state before anything else.
   - After a merge, wait for trunk's checks and every deploy workflow the merge commit triggers. If this change turned trunk or a deploy red, tell the user at once, then open a revert PR through these steps and merge it through the gate as soon as its checks are green, without waiting for reviewers. If the gate refuses the revert, the revert is the user's.
