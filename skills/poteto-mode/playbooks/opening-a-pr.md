...
4. Merge, when this PR is the whole task rather than a layer mid-stack or mid-phase. Run the **Babysit** playbook in `drive` mode until the watcher reports `READY`, then `scripts/merge-gate/merge-gate <number> --merge`. A refusal is the answer: report its reasons and stop. It refuses repos outside `~/.claude/pstack/merge-policy.json`, human-only paths without a human approval on the head, and anything not green. After a merge, read trunk's checks on the merge commit once they finish. If this change turned trunk red, open a revert PR through these steps and tell the user.
