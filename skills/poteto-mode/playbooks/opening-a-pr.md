...
3. No blockers: `gh pr create` without `--draft`, or `gh pr ready <number>` for your own draft that you have not readied before. A PR that went back to draft after you readied it is a hold: report it and stop. In a stack, ready only your own PR. `gh stack submit --auto --open` is for one agent that gated every PR in the stack, followed by `gh pr edit` for each title and body. With blockers: the PR stays or opens as a draft.
4. Merge, when this PR is the whole task rather than a layer mid-stack or mid-phase, and never for a revert PR. Run the **Babysit** playbook in `drive` mode until the watcher reports `READY`, then `scripts/merge-gate/merge-gate <number> --merge`. Never run `gh pr merge` yourself. A final refusal is never overridden that way: the user merges it.
   ...
   - After a merge, read the merge commit from the verdict, or from `gh pr view` when the verdict's is null. Wait for trunk's checks and every deploy workflow the merge commit triggers. If a run on the merge commit itself is red, compare the same workflow's latest completed run on the merge commit's first parent. If that was red too, report the failure as pre-existing and open no revert. Otherwise tell the user at once, and open a revert PR through steps 1 to 3 (never step 4) for the user to merge. Report its URL, any blocker, and any bot thread that names a deploy or data-compatibility risk. Never merge a revert yourself, because a red run can come from outside the change. A run that ends `cancelled` or skipped after the merge is pending: it takes the result of the same workflow's first completed run on a later trunk commit that contains the merge commit and does not revert it. If that run is red, report it as possibly either commit's and open no revert. If no such run completes, report the merge commit's status as unknown. A deploy waiting on a human approval is the user's: report it.
5. Reply with the PR URL, what the gate found, fixed, and dismissed, the Consider findings, any blocker, the merge result or refusal, and any revert PR with its blockers.
