...
Core discipline: one change, one measurement, keep or revert. Never stack untested changes, and never claim a win from code inspection (**principle-prove-it-works**).
...
5. Loop, one hypothesis per iteration:
   - Hand the change to a `pstack:poteto-agent` subagent on the `hillclimb` model from `references/models.md`, with a tight scope. Supervise and review the diff rather than typing it (**principle-guard-the-context-window**). When several independent hypotheses are live, fan them to parallel subagents, each with `isolation: "worktree"` (**principle-separate-before-serializing-shared-state**).
   ...
   Each iteration ends in a check before the next begins (**principle-sequence-verifiable-units**). If the run is unattended, borrow only the wake mechanism from the Autonomous run playbook (`playbooks/autonomous-run.md`), not its stop rule.
