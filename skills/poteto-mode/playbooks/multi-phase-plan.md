...
3. Explore in subagents with `subagent_type: "pstack:poteto-agent"` and a model from `references/models.md` (**principle-guard-the-context-window**). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
4. Copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write the file under `~/.claude/projects/<slug>/pstack/docs/`, where `<slug>` is the working directory with every non-alphanumeric character replaced by `-`. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (**principle-sequence-verifiable-units**). Name the execution playbook in **How to read this**. Pick between `playbooks/autopilot-full.md` and `playbooks/autopilot-stack.md` per the rule at the end of `playbooks/autopilot-stack.md`. A standing program takes `playbooks/orchestrate.md`.
...
6. Run `node scripts/check-plan.mjs <plan.md>` from poteto-mode's directory and fix every line it prints (**principle-encode-lessons-in-structure**).
...
**Control skill.** Pick it by surface. Browser and web UIs use the `claude-in-chrome` browser tools. Electron apps, CLIs, and TUIs use the built-in `run` skill. iOS uses the iOS Simulator tool. Other native mobile uses whatever simulator-driving skill the repo has. A repo-local verification skill, such as one made by **create-verification-skill**, beats all of these. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
The program runs poteto-mode's `playbooks/<execution playbook>.md`. <Who merges, and which PR ids are the operator's items that stop at merge-ready.>
...
- [ ] On the operator's go, the operator arms `/goal` with this exact text. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
- [ ] Read these at program start. Re-read them at every tick.
  - [ ] poteto-mode's `playbooks/<execution playbook>.md`
  - [ ] The **swarm** skill's `SKILL.md`
  - [ ] `git show origin/main:<control skill path>`, or the control skill's name when it ships outside the repo
  - [ ] poteto-mode's `playbooks/opening-a-pr.md`
  - [ ] The `SKILL.md` of each other pstack skill the program uses
- [ ] Arm the 30-minute audit tick with `/loop 30m` and the tick prompt below. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook and the /goal objective. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then post a short status message to the operator in chat only when the audit found a tracked change that no earlier status message reported, such as a PR opened, a code-ready head, a round launched or closed, a verdict, a merge, a stuck agent and the action taken, a blocker added or cleared, or a decision only the operator can make. Name every such change and nothing else. Do not repeat a table, the merged list, or an unchanged blocker. If the audit found none, end the turn with no reply text. Either way, log this tick's row in your decision trail. The row names the items reported, or none."
...
- [ ] Open the PR ready with `gh pr create --base <base-branch>`. A stack child targets its parent branch through `gh stack submit --auto`, then `gh pr ready`.
...
Each live lane runs in its own worktree at the PR head. Drive through the control skill this plan names.
...
<Docs to read before editing. Which PRs get the **how** skill. The trail per the **show-me-your-work** skill.>
````
