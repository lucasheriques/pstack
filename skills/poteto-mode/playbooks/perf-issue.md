...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a `pstack:poteto-agent` subagent on the `perf-issue` model from `references/models.md`. Review the diff. Capture a post-fix trace.
