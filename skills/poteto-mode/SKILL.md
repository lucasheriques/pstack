---
name: poteto-mode
description: poteto's agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Use at the start of any engineering task that matches a playbook (bug fix, feature, refactor, perf, investigation, PR babysit or shipping, multi-phase or autonomous runs) or needs rigor, and for /poteto-mode or requests for poteto's style. Skip it for casual turns or when the user opts out.
---
...
- Before commit → the built-in **simplify** skill (`/simplify`) over the diff.
...
Read the principle's file in full for any principle you apply. Each lives at `principles/<name>.md` in this skill's directory, for example `principles/principle-laziness-protocol.md`. Each entry names when it applies.
...
**Use `subagent_type: "pstack:poteto-agent"` for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). It preloads this skill. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review. Respect what the skill prescribes, don't override to `poteto-agent`.
...
- **Shipping.** The half after Babysit, on an explicit instruction to land. Independently verifying a green stack, then landing the contiguous verified run bottom-up through `gh`, with `gh stack sync` after each merge. `playbooks/shipping.md`.
...
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a transcript, remote session URL, or pushed branch. `playbooks/session-pickup.md`.
