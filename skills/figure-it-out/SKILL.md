...
Open a todolist whose first item is to read the Principles section of the **poteto-mode** skill (`${CLAUDE_SKILL_DIR}/../poteto-mode/SKILL.md`). Each `principle-<name>` below is a file in its `principles/` directory. Then add the phases below as todos.

## Phase A: Frame

Ground first, then commit. Don't start the run until you can state:

- The definition of done as a falsifiable predicate (**principle-prove-it-works**).
...
Decompose into atomic, independently-landable units. Sequence riskiest-unknown-first. Scaffold and verification come before features (**principle-foundational-thinking**).

- Build the verification harness before the work, with the baseline captured from the pre-change state, so the check reads as "old value vs new value".
- For one-way-door design decisions, run the **architect** skill (`${CLAUDE_SKILL_DIR}/../architect/SKILL.md`, it runs **arena**). Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (**principle-laziness-protocol**).
...
Log the run via the **show-me-your-work** skill (`${CLAUDE_SKILL_DIR}/../show-me-your-work/SKILL.md`). figure-it-out's work is usually ambitious enough to commit the trail so the reviewer can read it in the PR. The trail plus the diff is what lets the human come back and trust the work.

## Phase E: Verify and hand back

Check the whole against the Phase A predicate on the real product, not just the harness. Encode any recurring correction as a gate, a lint rule, a check, or a script (**principle-encode-lessons-in-structure**).
